...
- Optional: pick the virtual screen size, e.g. `cargo run --release -p extender-host -- 0.0.0.0:9000 2560x1440`.
...
| Choppy / high CPU | Software decode is CPU-bound; make sure you used `--release`. Try a smaller host resolution (step 3). |
